...
%   Modified 23 March 2021 (JEK) - changed to linear from spline interpolation
...
zamp_array = fitsread([polfile '_amp.fits']);
zpha_array = fitsread([polfile '_pha.fits']);
s = size(zamp_array);
nlam = s(1);
if ( nlam == 6 ); dlam = 100; else; dlam = 50; end
lam_array_m = (450:dlam:950)*1e-9;

%--Interpolate to get zernike coefficients at the specified wavelength
zamp = zeros(22,1);
zpha = zeros(22,1);
for iz = 1:22 
    zamp(iz) = interp1( lam_array_m, squeeze(zamp_array(:,iz,dir_in,dir_out)), lambda_m );
    zpha(iz) = interp1( lam_array_m, squeeze(zpha_array(:,iz,dir_in,dir_out)), lambda_m );
end
