...
function GdmTTlam = model_Jacobian_LC(mp, DM, tsi, whichDM)
...
%--DM1---------------------------------------------------------
if(whichDM==1) 
    GdmTTlam = zeros(mp.F4.compact.Ncorr,length(DM.dm1.act_ele) );
    ...
    for iarray=1:length(DM.dm1.act_ele)   %DM.dm1.compact.NactTotal  %--Only compute for acutators specified for use
        iact = DM.dm1.act_ele(iarray);
        ...
        EP4noFPM = zeros(DM.dm1.compact.NdmPad);
        ...
        GdmTTlam(:,iarray) = EF4(mp.F4.compact.corr.inds)/sqrt(mp.F4.compact.I00(modvar.sbpIndex));
...
%--DM2---------------------------------------------------------
if(whichDM==2)
    GdmTTlam = zeros(mp.F4.compact.Ncorr,length(DM.dm2.act_ele));
    ...
    for iarray=1:length(DM.dm2.act_ele)   %DM.dm1.compact.NactTotal  %--Only compute for acutators specified for use
        iact = DM.dm2.act_ele(iarray);
        ...
        EP4noFPM = zeros(DM.dm2.compact.NdmPad);
        EP4noFPM(y_box_ind,x_box_ind) = dEP2box; %--Propagating the E-field from P2 to P4 without masks gives the same E-field.
