...
function Gzdl = model_JacobianCP_HLC(mp, im, whichDM)
...
if(whichDM==5) 
    ...
    Edm2WFEpad = padOrCropEven(Edm2WFE,mp.dm5.compact.NdmPad); %--Pad or crop for expected sub-array indexing
...
if(whichDM==1) 
    ...
    for iact=mp.dm1.act_ele(:).'  %--MUST BE A COLUMN VECTOR `
        if( any(any(mp.dm1.compact.inf_datacube(:,:,iact))) )
            ...
            %--MFT from pupil P3 to FPM
%      JLlop       EF3 = rect_mat_pre*dEP3box*rect_mat_post; % MFT to FPM
%      JLlop       EF3 = (transOuterFPM-FPM).*EF3; %--Propagate through (1-complex FPM) for Babinet's principle

            %--DFT to LS ("Sub" name for Subtrahend part of the Lyot-plane E-field)
            EP4sub = 0;%JLlop propcustom_mft_FtoP(EF3,mp.fl,lambda,mp.F3.compact.dxi,mp.F3.compact.deta,mp.P4.compact.dx,mp.P4.compact.Narr,mp.centering);  %--Subtrahend term for the Lyot plane E-field    
            ...
            % DFT to camera
            EFend = propcustom_mft_PtoF(EP4,mp.fl,lambda,mp.P4.compact.dx,mp.Fend.dxi,2,mp.Fend.deta,2,mp.centering);
            if(mp.useGPU); EFend = gather(EFend) ;end

%             cp = max(max(abs(EFend).^2)); %JLlop
            cp = EFend(2,2); %JLlop
%             Gzdl(:,Gindex) = mp.dm1.weight*EFend(mp.Fend.corr.inds)/sqrt(mp.Fend.compact.I00(modvar.sbpIndex));
            Gzdl(:,Gindex) = mp.dm1.weight*cp/sqrt(mp.Fend.compact.I00(modvar.sbpIndex));
        end
        Gindex = Gindex+1;
    end
...
%             Gzdl(:,Gindex) = mp.dm2.weight*EFend(mp.Fend.corr.inds)/sqrt(mp.Fend.compact.I00(modvar.sbpIndex));
            Gzdl(:,Gindex) = mp.dm2.weight*cp/sqrt(mp.Fend.compact.I00(modvar.sbpIndex));
        end
        Gindex = Gindex+1;
...
%             end            
%   
% %          JLlop   dEF3box = ( (transOuterFPM-FPMpoked) - (transOuterFPM-FPM(eta_box_ind,xi_box_ind)) ).*EF3inc(eta_box_ind,xi_box_ind); % Delta field (in a small region) at the FPM
%             dEF3box =EF3inc(eta_box_ind,xi_box_ind); % Delta field (in a small region) at the FPM
% 
...
%             Gzdl(:,Gindex) = (1/stepFac)*mp.dm8.weight*cp/sqrt(mp.Fend.compact.I00(modvar.sbpIndex));
%             Gindex = Gindex + 1;
%         end
%     end
% 
