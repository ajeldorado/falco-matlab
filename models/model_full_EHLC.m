...
function Eout = model_full_EHLC(mp,   lambda, normFac, Ein, FPM)  
% function Eout = model_full_EHLC(mp,   modvar)
...
% ilam = (modvar.sbpIndex-1)*mp.Nwpsbp + modvar.wpsbpIndex; %--Wavelength index for full model
...
%     FPM = falco_gen_EHLC_FPM_complex_trans_mat( mp,modvar.sbpIndex,modvar.wpsbpIndex,'full');
% end
...
%--MFT from apodizer plane to FPM (i.e., P3 to F3)
    EF3 = propcustom_mft_PtoF(EP3, mp.fl,lambda,mp.P2.full.dx,mp.F3.full.dxi,mp.F3.full.Nxi,mp.F3.full.deta,mp.F3.full.Neta,mp.centering);

%--Don't apply FPM if normalization value is being found
if(normFac==0)
    EF3 = transOuterFPM.*EF3;
else
    EF3 = FPM.*EF3; %--Apply FPM
end 
...
EP4 = mp.P4.full.croppedMask.*EP4; %--Apply Lyot stop
