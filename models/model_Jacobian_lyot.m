...
% OUTPUTS
% ------
% Gchunk : Jacobian for the specified Zernike mode, DM, star, subband, and
% DM actuator.

function Gchunk = model_Jacobian_lyot(mp, imode, whichDM, iact)
...
if whichDM == 2
    ...
    if mp.flagFiber
        if mp.flagLenslet
            ...
            EFend = propcustom_mft_PtoF(EP4, mp.fl, lambda, mp.P4.compact.dx, mp.Fend.dxi, mp.Fend.Nxi, mp.Fend.deta, mp.Fend.Neta, mp.centering);
            Gmodetemp = zeros(mp.Fend.Nfiber, 1);
            for ii=mp.Fend.Nfiber:-1:1
                Eonefiber = sum(sum(mp.Fend.fiberMode(:, :, modvar.sbpIndex, ii).*EFend)) / sqrt(mp.Fend.compact.I00_fiber(ii, modvar.sbpIndex));
                Gmodetemp(ii) = Eonefiber;
            end
            Gchunk(ii) = Gmodetemp;
        end
    else    
