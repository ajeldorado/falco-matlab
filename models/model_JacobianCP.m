...
% REVISION HISTORY:
% --------------
% Modified on 2019, Feb 28 by Jorge Llop, to build the Jacobian for
% introducing the throughput in the cost function of EFC. CP stands for
% central pixel.
...
function jacStruct = model_JacobianCP(mp)
    ...
    if(any(mp.dm_ind==1)); jacStruct.G1 = zeros(1,mp.dm1.Nele,mp.jac.Nmode);  else;  jacStruct.G1 = zeros(0,0,mp.jac.Nmode);  end % control Jacobian for DM1
    if(any(mp.dm_ind==2)); jacStruct.G2 = zeros(1,mp.dm2.Nele,mp.jac.Nmode);  else;  jacStruct.G2 = zeros(0,0,mp.jac.Nmode);  end % control Jacobian for DM2
    if(any(mp.dm_ind==3)); jacStruct.G3 = zeros(1,mp.dm3.Nele,mp.jac.Nmode);  else;  jacStruct.G3 = zeros(0,0,mp.jac.Nmode);  end % control Jacobian for DM3
    if(any(mp.dm_ind==4)); jacStruct.G4 = zeros(1,mp.dm4.Nele,mp.jac.Nmode);  else;  jacStruct.G4 = zeros(0,0,mp.jac.Nmode);  end % control Jacobian for DM4
    if(any(mp.dm_ind==5)); jacStruct.G5 = zeros(1,mp.dm5.Nele,mp.jac.Nmode);  else;  jacStruct.G5 = zeros(0,0,mp.jac.Nmode);  end % control Jacobian for DM5
    if(any(mp.dm_ind==6)); jacStruct.G6 = zeros(1,mp.dm6.Nele,mp.jac.Nmode);  else;  jacStruct.G6 = zeros(0,0,mp.jac.Nmode);  end % control Jacobian for DM6
    if(any(mp.dm_ind==7)); jacStruct.G7 = zeros(1,mp.dm7.Nele,mp.jac.Nmode);  else;  jacStruct.G7 = zeros(0,0,mp.jac.Nmode);  end % control Jacobian for DM7
    if(any(mp.dm_ind==8)); jacStruct.G8 = zeros(1,mp.dm8.Nele,mp.jac.Nmode);  else;  jacStruct.G8 = zeros(0,0,mp.jac.Nmode);  end % control Jacobian for DM8
    if(any(mp.dm_ind==9)); jacStruct.G9 = zeros(1,mp.dm9.Nele,mp.jac.Nmode);  else;  jacStruct.G9 = zeros(0,0,mp.jac.Nmode);  end % control Jacobian for DM9
    ...
    if(mp.flagParfor) 
        parfor ii=1:Nvals
            Jtemp{ii} = model_JacobianCP_middle_layer(mp, vals_list, ii);
        end
    ...
    %--Regular computing (avoid using variable Jtemp to save RAM
        ...
        for ii=1:Nvals
            ...
            if(whichDM==1); jacStruct.G1(:,:,im) =  model_JacobianCP_middle_layer(mp, vals_list, ii);  end
            if(whichDM==2); jacStruct.G2(:,:,im) =  model_JacobianCP_middle_layer(mp, vals_list, ii);  end
            if(whichDM==5); jacStruct.G5(:,:,im) =  model_JacobianCP_middle_layer(mp, vals_list, ii);  end
            if(whichDM==8); jacStruct.G8(:,:,im) =  model_JacobianCP_middle_layer(mp, vals_list, ii);  end
            if(whichDM==9); jacStruct.G9(:,:,im) =  model_JacobianCP_middle_layer(mp, vals_list, ii);  end
        end
        fprintf('\n')
    end    
...
function jacMode = model_JacobianCP_middle_layer(mp,   vals_list,ii)
    ...
    switch upper(mp.coro) 
        ...
        case{'HLC','APHLC'} %--DMs, optional apodizer, FPM with phase modulation, and LS.
            jacMode = model_JacobianCP_HLC(mp,   im, whichDM); 
