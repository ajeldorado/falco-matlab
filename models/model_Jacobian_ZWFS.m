...
function Gzdl = model_Jacobian_ZWFS(mp, im, whichDM)
...
%--DM1---------------------------------------------------------
if(whichDM==1) 
    Gzdl = zeros(mp.P4.compact.Narr^2,mp.dm1.Nele);
    ...
    for iact=mp.dm1.act_ele(:).'
        if(any(any(mp.dm1.compact.inf_datacube(:,:,iact))) && any(mp.dm1.act_ele==iact) )  %--Only compute for acutators specified for use or for influence functions that are not zeroed out
            ...
            EP4residual = EP4 - mp.ZWFSreferencewave(:,:,modvar.sbpIndex);
            
            if(mp.useGPU);EP4residual = gather(EP4residual);end
            
            Gzdl(:,Gindex) = reshape(EP4residual, [], 1);
        end
        Gindex = Gindex + 1;
    end
end
...
if(whichDM==2) 
    Gzdl = zeros(mp.P4.compact.Narr^2,mp.dm2.Nele);
    Gindex = 1; % initialize index counter
    for iact=mp.dm2.act_ele(:).'
        if(any(any(mp.dm1.compact.inf_datacube(:,:,iact))) && any(mp.dm2.act_ele==iact) )  %--Only compute for acutators specified for use or for influence functions that are not zeroed out
            Gzdl(:,Gindex) = 0;
        end
        Gindex = Gindex + 1;
    end
