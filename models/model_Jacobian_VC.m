...
function Gchunk = model_Jacobian_VC(mp, imode, whichDM, iact)
...
if whichDM == 1
    ...
    % if mp.flagFiber
    %     if mp.flagLenslet
    %         Gchunk = zeros(mp.Fend.Nlens, 1);
    %     else
    %         Gchunk = zeros(mp.Fend.Nfiber, 1);
    %     end
    % else
    %     Gchunk = zeros(mp.Fend.corr.Npix, 1); %--Initialize the Jacobian
    % end
    ...
    if mp.flagFiber
        if mp.flagLenslet
            for ilens = mp.Fend.Nlens:-1:1
                EFend = propcustom_mft_PtoF(EP4, mp.fl, lambda, mp.P4.compact.dx, mp.Fend.dxi, mp.Fend.Nxi, mp.Fend.deta, mp.Fend.Neta, mp.centering, 'xfc', mp.Fend.x_lenslet_phys(ilens), 'yfc', mp.Fend.y_lenslet_phys(ilens));
                ...
                Gchunk(ilens) = max(max(mp.F5.fiberMode(:, :, modvar.sbpIndex))).*sum(sum(mp.F5.fiberMode(:, :, modvar.sbpIndex).*EF5));
            end
        else
            ...
            for ifiber = mp.Fend.Nfiber:-1:1
                Eonefiber = sum(sum(mp.Fend.fiberMode(:, :, modvar.sbpIndex, ifiber).*EFend)) / sqrt(mp.Fend.compact.I00_fiber(ifiber,modvar.sbpIndex));
                Gmodetemp(ifiber) = Eonefiber;
            end
            Gchunk = Gmodetemp;
        end
    else    
        ...
        Gchunk = EFend(mp.Fend.corr.maskBool) / sqrt(mp.Fend.compact.I00(modvar.sbpIndex));
    end
    
    if mp.jac.minimizeNI
       JacOfPeak = model_Jacobian_no_FPM(mp, imode, whichDM, iact); 
       Gchunk = Gchunk/Epeak - Eocculted(mp.Fend.corr.maskBool) / (Epeak*Epeak) .* JacOfPeak;
    end
    
    Gchunk = mp.dm1.weight * Gchunk;
end    
...
if whichDM == 2
    ...
    % if mp.flagFiber
    %     if mp.flagLenslet
    %         Gchunk = zeros(mp.Fend.Nlens, mp.dm2.Nele);
    %     else
    %         Gchunk = zeros(mp.Fend.Nfiber, mp.dm2.Nele);
    %     end
    % else
    %     Gchunk = zeros(mp.Fend.corr.Npix, mp.dm2.Nele); %--Initialize the Jacobian
    % end
