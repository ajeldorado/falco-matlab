...
%
% REVISION HISTORY:
% - Modified on 2019-07-08 by A.J. Riggs to be for EFC that minimizes total
% stroke as well.
...
function [dDM,cvar] = falco_ctrl_planned_EFC_TS(mp, cvar)
    ...
    if(any(mp.gridSearchItrVec==cvar.Itr))
        ...
        if(mp.flagParfor) %--Parallelized
            parfor ni = 1:Nvals
                [Inorm_list(ni),dDM_temp] = falco_ctrl_EFC_TS_base(ni,vals_list,mp,cvar);
                %--Tied actuators
    ...
    if( (imag(mp.ctrl.log10regSchedIn(cvar.Itr)) ~= 0) && (real(mp.ctrl.log10regSchedIn(cvar.Itr)) == 0) && any(mp.gridSearchItrVec==cvar.Itr) )
        ...
        [cvar.cMin,dDM] = falco_ctrl_EFC_TS_base(ni,vals_list,mp,cvar);
        if(mp.ctrl.flagUseModel)
