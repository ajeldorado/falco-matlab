...
% tableRaw = falco_FRN_InitialRawContrast(mp);
...
for ia=1:Nann        
    ...
    figure(401); imagesc(maskPartial.*Ccoh); axis xy equal tight; colorbar;
    pause(1);

end

figure(411); semilogy(rVec,CcohVec,'-kx',rVec,CincoVec,':kx')

%% Fill in Column 5 of the table

%--Coherent component
tableRaw(1:2:2*Nann,5) = 2*CcohVec; %--Coherent, MUF=2xCoherent
tableRaw(2*Nann+1:2:end,5) = CcohVec; %--Coherent, no MUF

%--NOTICE: UNKNOWN YET WHETHER THE "incoherent" PART OF THE TABLE INCLUDES
%THE COHERENT PART TOO, AND HOW MUFs ARE IMPLEMENTED.
%--Incoherent
tableRaw(2:2:2*Nann,5) = 2*CincoVec; %--Incoherent+Coherent, MUFs of ???????? %--NEED TO FIND OUT HOW MUFS ARE CALCULATED. THIS IS A PLACEHOLDER
tableRaw(2*Nann+2:2:end,5) = CincoVec; %--Incoherent+Coherent, no MUF
