...
mp.ctrl.sched_mat = [...
    repmat([1,1j,12,0,1],[1,1]);...   %--Optimal beta
    ];
...
mp.full.input_field_rootname = '/home/ajriggs/Repos/falco-matlab/data/maps/input_full'; %--Full rootname of the input E-field files that will be saved
...
%--[OPTIONAL] Start from a previous FALCO trial's DM settings
fn_prev = 'Series0049_Trial0001_HLC_WFIRST180718_2DM48_z1_IWA2.7_OWA9_3lams575nm_BW10_plannedEFC_snippet.mat';
temp = load(fn_prev,'out');
mp.dm1.V = temp.out.DM1V;
mp.dm2.V = temp.out.DM2V;
clear temp
...
if( strcmp(mp.path.falco(end),'/')==false && strcmp(mp.path.falco(end),'\')==false ); mp.path.falco = [mp.path.falco filesep]; end
%--Store minimal data to re-construct the data from the run: the config files and "out" structure after a trial go here
if(isfield(mp.path,'config')==false); mp.path.config = [mp.path.falco filesep 'data' filesep 'config' filesep]; end
...
fn_config = [mp.path.config mp.runLabel,'_configYield.mat'];
save(fn_config)
