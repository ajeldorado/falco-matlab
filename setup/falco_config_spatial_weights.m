...
% mp.WspatialFull = mp.Fend.corr.mask;
if( isfield(mp,'WspatialDef') ) %--If there are spatial weights for the Jacobian
    if(isempty(mp.WspatialDef)==false)
        ...
        for kk=1:size(mp.WspatialDef,1)
            ...
            %if(mp.flagPlot); figure(70); imagesc(mp.Fend.xisDL,mp.Fend.etasDL,WannulusCompact); axis xy equal tight; colorbar; pause(1); end
            
            mp.Wspatial = mp.Wspatial.*Wannulus;
            %mp.WspatialFull = mp.WspatialFull.*WannulusFull;
            %if(mp.flagPlot); figure(71); imagesc(mp.Fend.xisDL,mp.Fend.etasDL,mp.Wspatial); axis xy equal tight; colorbar; drawnow; end
        end
    else
        %--nothing
    end
