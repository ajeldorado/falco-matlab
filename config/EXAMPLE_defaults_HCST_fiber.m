...
if flag_bigDM
    ...
    mp.dm1.xc = (mp.dm1.Nact/2 - 1/2);       % x-center location of DM surface [actuator widths]
    mp.dm1.yc = (mp.dm1.Nact/2 - 1/2);       % y-center location of DM surface [actuator widths]
    mp.dm2.VtoH = 1e-9*ones(mp.dm2.Nact);  % gains of all actuators [nm/V of free stroke]
    mp.dm2.xc = (mp.dm2.Nact/2 - 1/2);       % x-center location of DM surface [actuator widths]
    mp.dm2.yc = (mp.dm2.Nact/2 - 1/2);       % y-center location of DM surface [actuator widths]
end
...
% mp.Fend.corr.ang  = 150;  % angular opening of dark hole correction region [degrees]
...
if strcmp(mp.whichPupil,'LUVOIR_B')
    % Full model:
...
elseif strcmp(mp.whichPupil,'Simple')
    % Full model:
    inputs.Nbeam = mp.P1.full.Nbeam;
    inputs.Npad = 2^(nextpow2(inputs.Nbeam));
    inputs.OD = 1;
    mp.P1.full.mask = pad_crop(falco_gen_pupil_Simple(inputs), 2^(nextpow2(inputs.Nbeam)));
    % Compact model
    inputs.Nbeam = mp.P1.compact.Nbeam;
    inputs.Npad = 2^(nextpow2(inputs.Nbeam));
    inputs.OD = 1;
    mp.P1.compact.mask = pad_crop(falco_gen_pupil_Simple(inputs), 2^(nextpow2(inputs.Nbeam)));
end
...
%% Wavefront Errors
if flag_wfe
    num_z = length(indsZnoll_arr);
    sz_f = size(mp.P1.full.mask);
    ZmapCube_full = falco_gen_norm_zernike_maps(sz_f(1)-2,'pixel',indsZnoll_arr);
    sz_c = size(mp.P1.compact.mask);
    ZmapCube_compact = falco_gen_norm_zernike_maps(sz_c(1)-2,'pixel',indsZnoll_arr);
%     mp.P1.full.E = zeros(mp.P1.full.Nbeam+1,mp.P1.full.Nbeam+1,Nsbp,);
%     mp.P1.compact.E = zeros(mp.P1.compact.Nbeam+1);
    phase_map_full = zeros(sz_f);
    phase_map_compact = zeros(sz_c);
    for II=1:num_z
        phase_map_full = phase_map_full+ZmapCube_full(:,:,II)*wfe_arr(II);
        phase_map_compact = phase_map_compact+ZmapCube_compact(:,:,II)*wfe_arr(II);
    end
    mp.P1.full.mask = mp.P1.full.mask.*exp(2*pi*1i*phase_map_full/(mp.lambda0*1e9));
    mp.P1.compact.mask = mp.P1.compact.mask.*exp(2*pi*1i*phase_map_compact/(mp.lambda0*1e9));
    clear ZmapCube_full ZmapCube_compact
end
% figure(103)
% imagesc(angle(mp.P1.full.mask))
% colorbar
% dsadsda