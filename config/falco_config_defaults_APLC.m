...
switch mp.whichPupil
    case 'Simple' % Can be used to create circular and annular apertures with radial spiders 
        ...
        mp.P1.Dfac = 1; %--meters, circumscribing diameter of telescope (used for scaling pupil and FPM mask sizes)
    ...
    case{'WFIRST_onaxis'}
        mp.P1.Dfac = 1; %--Ratio of OD_circumscribed to OD_inscribed for the non-circular outer aperture.
        mp.P1.D = 2.3631; %--meters, diameter of telescope (used only for mas to lambda/D conversion)
        if(isfield(mp,'pup_strut_width')==false); mp.pup_strut_width = 0.0261; end  %--Old value was 2.61% in pre-Phase A. 3.22% is the new value as of 2018-01-03
        ...
        if(isfield(mp.P1.compact,'Nbeam')==false); mp.P1.compact.Nbeam = 1000; end 
    case{'WFIRST20180103'}
        mp.P1.Dfac = 1; %--Ratio of OD_circumscribed to OD_inscribed for the non-circular outer aperture.
        mp.P1.D = 2.3631; %--meters, diameter of telescope (used only for mas to lambda/D conversion)
    ...
    case{'LUVOIRA0'}
        mp.P1.D = 15.022; %--meters, circumscribing diameter of telescope (used for scaling pupil and FPM mask sizes)
        mp.P1.Dfac = mp.P1.D/12.644; %--Ratio of OD_circumscribed to OD_inscribed for the non-circular outer aperture.
        
        if(isfield(mp.P1.full,'Nbeam')==false); mp.P1.full.Nbeam = 1000; end %--Number of pixels across the actual diameter of the beam/aperture (independent of beam centering
        if(isfield(mp.P1.compact,'Nbeam')==false); mp.P1.compact.Nbeam = 800; end 
    ...
    case{'LUVOIRA5'}
        ...
        if(isfield(mp.P4.full,'Nbeam')==false); mp.P4.full.Nbeam = 400; end 
        if(isfield(mp.P4.compact,'Nbeam')==false); mp.P4.compact.Nbeam = 400; end
...
% if(strcmpi(mp.SPname,'20170714'))
%     if(isfield(mp.F4.corr,'Rin')==false); mp.F4.corr.Rin  = 2.6; end  %--lambda0/D, inner radius of correction region
%     if(isfield(mp.F4.score,'Rin')==false); mp.F4.score.Rin = 2.9; end  %--Needs to be >= that of Correction mask
%     if(isfield(mp.F4.corr,'Rout')==false); mp.F4.corr.Rout  = 9; end %floor(mp.dm1.Nact/2*(1-mp.fracBW/2)); end  %--lambda0/D, outer radius of correction region
%     if(isfield(mp.F4.score,'Rout')==false); mp.F4.score.Rout = 8.7; end  %--Needs to be <= that of Correction mask
%     if(isfield(mp.F4.corr,'ang')==false); mp.F4.corr.ang  = 65; end  %--degrees per side
%     if(isfield(mp.F4.score,'ang')==false); mp.F4.score.ang = 60; end  %--degrees per side
% elseif(strcmpi(mp.SPname,'32WA194'))
...
%     if(isfield(mp.F4.corr,'Rout')==false); mp.F4.corr.Rout  = 19.4; end %floor(mp.dm1.Nact/2*(1-mp.fracBW/2)); end  %--lambda0/D, outer radius of correction region
...
%     if(isfield(mp.F4.corr,'Rout')==false); mp.F4.corr.Rout  = 22.0; end %floor(mp.dm1.Nact/2*(1-mp.fracBW/2)); end  %--lambda0/D, outer radius of correction region
%     if(isfield(mp.F4.score,'Rout')==false); mp.F4.score.Rout = mp.F4.corr.Rout; end  %--Needs to be <= that of Correction mask
%     if(isfield(mp.F4.corr,'ang')==false); mp.F4.corr.ang  = 180; end  %--degrees per side
%     if(isfield(mp.F4.score,'ang')==false); mp.F4.score.ang = 180; end  %--degrees per side
...
%     if(isfield(mp.F4.score,'Rin')==false); mp.F4.score.Rin = mp.F4.corr.Rin; end  %--Needs to be >= that of Correction mask
%     if(isfield(mp.F4.corr,'Rout')==false); mp.F4.corr.Rout  = min( [26.4*mp.P1.Dfac, floor(mp.dm1.Nact/2*(1-mp.fracBW/2)) ]) ; end %--lambda0/Dcircumscribed, outer radius of correction region
%     if(isfield(mp.F4.score,'Rout')==false); mp.F4.score.Rout = mp.F4.corr.Rout; end  %--Needs to be <= that of Correction mask
%     if(isfield(mp.F4.corr,'ang')==false); mp.F4.corr.ang  = 180; end  %--degrees per side
%     if(isfield(mp.F4.score,'ang')==false); mp.F4.score.ang = 180; end  %--degrees per side
% end
