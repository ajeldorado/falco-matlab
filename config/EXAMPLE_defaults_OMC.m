...
mp.dm_ind = [1]; % vector of DMs used in controller at ANY time (not necessarily all at once or all the time). 
mp.ctrl.dmfacVec = 1;
...
%mp.ctrl.sched_mat = [...
%    repmat([1,1j,1,1,1],[4,1]);...
%    repmat([1,1j-1,1,1,1],[25,1]);...
%    repmat([1,1j,1,1,1],[1,1]);...
%    ];
mp.ctrl.sched_mat = [...
    repmat([1,1j,1,1,1],[200,1]);...
    ];
...
mp.dm2.Dstop = 100e-3;   %--Diameter of iris [meters]
...
mp.Fend.score.Rout = 10.0;  % outer radius of dark hole scoring region [lambda0/D]
...
%mask_pn = '/Users/cprada/Desktop/HCIT_Operator/OMC/MSWFC/SPM_SPC-20200623_982_rounded9_gray_rotated.fits'


%% Lyot stop (P4) Definition and Generation


mask_Lyot_raw = fitsread('LS_SPC-20200623_491_rotated.fits');
mask_Lyot_sca = abs(imresize(mask_Lyot_raw,0.87* 300/446));
B=padarray(mask_Lyot_sca,(512-size(mask_Lyot_sca,1)+1)./2,0,'both');
mask_Lyot=padarray(B',(512-size(mask_Lyot_sca,1)+1)./2,0,'both');

% Full model
mp.P4.full.mask = mask_Lyot(1:512,1:512);

% Compact model
mp.P4.compact.mask = mask_Lyot(1:512,1:512);
