...
function mp = falco_config_defaults_AHLC(mp)
...
%% Apodizer (i.e., DM5) Properties
% %--Set the DM5 influence function
% if(isfield(mp.dm5,'inf0name'))
%     switch mp.dm5.inf0name
%         case '3x3'
%             mp.dm5.inf0 = 1/4*[1, 2, 1; 2, 4, 2; 1, 2, 1];  % influence function
%             mp.dm5.dx_inf0_act = 1/2;  % number of inter-actuator widths per pixel 
% %             %--FPM resolution (pixels per lambda0/D) in the compact and full models.
% %             mp.F3.compact.res = mp.dm5.actres/mp.dm5.dx_inf0_act;
% %             mp.F3.full.res = mp.dm5.actres/mp.dm5.dx_inf0_act;
%             
%     end
% end
...
%%--Final Focal Plane (F4) Properties
if(isfield(mp.F4,'res')==false); mp.F4.res = 3; end  %--Pixels per lambda_c/D
