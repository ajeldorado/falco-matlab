...
for iSubband = 1:mp.Nsbp
    ...
    for i = 0:floor(ev.SL/ev.BS)-1
        ev.Q(:,:,i+1,iSubband) = G_reordered((i)*ev.BS+1:(i+1)*ev.BS,:) * dm_drift_covariance * G_reordered(i*ev.BS+1:(i+1)*ev.BS,:).' * ev.sbp_texp(iSubband) * (ev.e_scaling(iSubband)^2);
    end
end
