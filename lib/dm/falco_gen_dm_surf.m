...
if(isfield(dm,'HminStep') && ~any(isnan(dm.HminStep)))
    ...
    % Quantization not desired; send raw voltage
    H = dm.VtoH.*dm.V;
end

if(isfield(dm,'inf_info'))
    [~,DMsurf] = propcustom_dm(bm, H, dm.xc-cshift, dm.yc-cshift, dm.dm_spacing,'XTILT',dm.xtilt,'YTILT',dm.ytilt,'ZTILT',dm.zrot,orderOfOps, 'custom_infl', dm.inf_info);
else
