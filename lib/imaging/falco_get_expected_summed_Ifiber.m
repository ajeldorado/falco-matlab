...
function [Ibandavg,Ifiberbandavg] = falco_get_expected_summed_Ifiber(mp, cvar)
    ...
    EfibernewTempVecArray = zeros(1, mp.jac.Nmode);
    EfiberoldTempVecArray = zeros(1, mp.jac.Nmode);
    ...
    for iMode = 1:mp.jac.Nmode    
        ...
        [Etemp,Efiber] = model_compact(mp, modvar);
        EnewTempVecArray(:, iMode) = Etemp(mp.Fend.corr.mask);
        EfibernewTempVecArray(1,iMode) = Efiber;
    end
    ...
    for iMode = 1:mp.jac.Nmode    
        ...
        EfiberoldTempVecArray(1,iMode) = Efiber;
    end
    
    %--Compute the expected new 2-D intensity image
    for iMode = 1:mp.jac.Nmode    
        EexpectedVec = cvar.Eest(iMode) + (EfibernewTempVecArray(1,iMode) - EfiberoldTempVecArray(1,iMode));
        
        sbpIndex = mp.jac.sbp_inds(iMode);
        Ifiberbandavg = Ifiberbandavg +  mp.sbp_weights(sbpIndex)*abs(EexpectedVec).^2;
    end
    Ibandavg = Ifiberbandavg;
end %--END OF FUNCTION
