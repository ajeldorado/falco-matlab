...
ev = initialize_modal_ekf_matrices(mp, ev, sbp_texp);
...
function ev = initialize_modal_ekf_matrices(mp, ev, sbp_texp)
...
ev.SL = ev.SS*mp.Fend.corr.Npix;%sum(size(jacStruct.G1,1));%Total length of the state vector (all pixels).
...
if any(mp.dm_ind == 2)
    num_actuators = mp.dm1.Nele+mp.dm2.Nele; 
else 
    num_actuators = mp.dm1.Nele; 
end

ev.Q = zeros(num_actuators, num_actuators, mp.Nsbp);
for iSubband = 1:1:mp.Nsbp
    disp(['assembling Q for subband ',num2str(iSubband)])

    ev.Q(:,:, iSubband) = eye(num_actuators) * mp.drift.presumed_dm_std^2;
end
...
ev.u_hat = zeros(num_actuators, mp.Nsbp);
