...
switch lower(mp.estimator)
    case{'pairwise', 'pairwise-square', 'pwp-bp-square', 'pairwise-rect', 'pwp-bp' , 'pairwise-bb'}
        clear ev
...
for k = 1:Npairs-1
    probePhaseVec = [probePhaseVec probePhaseVec(end)-(Npairs-1)]; %* appends the value of the last element of probePhaseVec minus (Npairs-1) to probePhaseVec
    probePhaseVec = [probePhaseVec probePhaseVec(end)+(Npairs)];
end
probePhaseVec = probePhaseVec * pi / Npairs; 

switch mp.estimator
    case{'pairwise', 'pairwise-square', 'pwp-bp-square', 'pwp-kf', 'pairwise-kf', 'pairwise-bb'}
...
%     end
    ...
    Iplus  = zeros([mp.Fend.corr.Npix, Npairs]); % Pixels of plus probes' intensities  (2D array)
    Iminus = zeros([mp.Fend.corr.Npix, Npairs]); % Pixels of minus probes' intensities
    if mp.flagFiber
        Ifiberplus  = zeros([mp.Fend.Nfiber, Npairs]); % Pixels of plus probes' intensities (2D array)
...
%         mp.Nwpsbp = mp.Nsbp; 
%         mp.Nsbp = 1;
%         disp('if strcmpi(mp.estimator, "pairwise-bb"), mp.Nwpsbp :')
%         disp(mp.Nwpsbp)
%         disp('if strcmpi(mp.estimator, "pairwise-bb"), mp.Nsbp :')
%         disp(mp.Nsbp)
        %mp = falco_set_spectral_properties(mp);
...
elseif strcmpi(mp.estimator, 'pairwise-bb') 
    ev.IincoEst =  I0vec - abs(Eest).^2; % incoherent light
else
...
if(isfield(ev,'amp_model')==true);  ev.amp_model = amp_model; end
