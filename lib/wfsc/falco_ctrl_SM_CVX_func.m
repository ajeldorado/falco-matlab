...
%         if(any(mp.dm_ind==1)); dDM.dDM1V = zeros(mp.dm1.Nact,mp.dm1.Nact);  dDM.dDM1V(mp.dm1.act_ele) = mp.dm_weights(1)*duVec(uLegend==1);   end
%         if(any(mp.dm_ind==2)); dDM.dDM2V = zeros(mp.dm2.Nact,mp.dm2.Nact);  dDM.dDM2V(mp.dm2.act_ele) = mp.dm_weights(2)*duVec(uLegend==2);   end
%         if(any(mp.dm_ind==8)); dDM.dDM8V = zeros(mp.dm8.NactTotal,1);  dDM.dDM8V(mp.dm8.act_ele) = mp.dm_weights(8)*duVec(uLegend==8);  end
%         if(any(mp.dm_ind==9)); dDM.dDM9V = zeros(mp.dm9.NactTotal,1);  dDM.dDM9V(mp.dm9.act_ele) = mp.dm_weights(9)*duVec(uLegend==9);  end        
%         
