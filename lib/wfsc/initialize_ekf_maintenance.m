...
for iSubband = 1:mp.Nsbp
    ...
    ev.peak_psf_counts(iSubband) = ev.sbp_texp(iSubband)*psf_peaks(iSubband);
...
for iSubband = 1:1:mp.Nsbp
    ...
    ev.x_hat0(1:ev.SS:end,iSubband) = real(E_hat) * ev.e_scaling(iSubband) * sqrt(ev.sbp_texp(iSubband));
    ev.x_hat0(2:ev.SS:end,iSubband) = imag(E_hat) * ev.e_scaling(iSubband) * sqrt(ev.sbp_texp(iSubband));

    % The EKF state is scaled such that the intensity is measured in photons:
    ev.x_hat(1:ev.SS:end,iSubband) = real(E_hat) * ev.e_scaling(iSubband) * sqrt(ev.sbp_texp(iSubband));
    ev.x_hat(2:ev.SS:end,iSubband) = imag(E_hat) * ev.e_scaling(iSubband) * sqrt(ev.sbp_texp(iSubband));
 end
