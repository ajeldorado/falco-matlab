% Produce both the 2-D representation and x and y points for a horizontal bowtie mask.
%
% Parameters
% ----------
% Nbeam : float 
...
function [mask, xyAllCell] = falco_gen_rounded_bowtie_horizontal_vector(Nbeam, ID, OD, rocFillet, angDeg, clockDeg, upsampleFactor, DbeamUM, stepSize, xc, yc, centering)

flagPlot = false;
flagPlotFinal = false;

rocFilletUM = rocFillet*DbeamUM; %--width of the dropout region [microns]

% Number of samples across in square output array
maxOffset = max(abs([xc, yc]))/DbeamUM*Nbeam;
switch centering
    case 'pixel'
...
for istrut = [3, 1] %nStrut:-1:1 % 1:nStrut % 
