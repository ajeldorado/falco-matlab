...
function out = falco_gen_rounded_bowtie_LS(Nbeam, ID, OD, Rfillet, upsampleFactor, angDeg, clockDeg, centering)
...
RfilletUM = Rfillet*DbeamUM; %255;%170.; %--width of the dropout region [microns]
...
nPointsFillet = round(2*pi*RfilletUM/stepSize);
...
%--Sideways openings
% mask = double(RHOS>=ID/2. & RHOS <= OD/2. & ((Y <= slopeB*X & Y >= -slopeA*X & X > 0) | (Y >= slopeB*X & Y <= -slopeA*X & X < 0)));
...
for iInterior = 1:length(grayInds)
    ...
    subpixel(RHOSup>=ID/2. & RHOSup <= OD/2. & ((Xup <= slopeB*Yup & Xup >= -slopeA*Yup & Yup > 0) | (Xup >= slopeB*Yup & Xup <= -slopeA*Yup & Yup < 0))) = 1;
...
for istrut = [1, 3] %nStrut:-1:1 % 1:nStrut % 
    ...
    m2 = strutSlopeVec(istrutB);
    bB = yOffsetVec(istrutB) + (strutWidthVec(istrutB)/2.0 + Rfillet)/cosd(strutAngleVec(istrutB));
    ...
    b1 = yOffsetVec(istrut) - (strutWidthVec(istrut)/2.0 + Rfillet)/cosd(strutAngleVec(istrut));
...
%     xyMask{istrut} = [xAll; yAll];
    ...
    for iFillet = 1:nFillet
        ...
        maskFillet = maskFillet | ( (pms*Y > 0) & (RHOSf.^2>=Rfillet.^2) & (pmf*Y >= pmf*(mSecantVec(iFillet)*X + bSecantVec(iFillet))) &...
            (RHOS>=ID/2. & RHOS <= OD/2. & ((X <= slopeB*Y & X >= -slopeA*Y & Y > 0) | (X >= slopeB*Y & X <= -slopeA*Y & Y < 0))));
%         maskFillet = maskFillet | ( (pms*X > 0) & (RHOSf.^2>=Rfillet.^2) & (pmf*Y >= pmf*(mSecantVec(iFillet)*X + bSecantVec(iFillet))) &...
%             (RHOS>=ID/2. & RHOS <= OD/2. & ((Y <= slopeB*X & Y >= -slopeA*X & X > 0) | (Y >= slopeB*X & Y <= -slopeA*X & X < 0))));
    ...
    for iFillet = 1:nFillet
        ...
        maskFillet = ( (pms*Y > 0) & (RHOSf.^2>=Rfillet.^2) & (pmf*Y >= pmf*(mSecantVec(iFillet)*X + bSecantVec(iFillet))) &...
            (RHOS>=ID/2. & RHOS <= OD/2. & ((X <= slopeB*Y & X >= -slopeA*Y & Y > 0) | (X >= slopeB*Y & X <= -slopeA*Y & Y < 0))));
        maskFillet = double(maskFillet);
        
%         maskFillet = ( (pms*X > 0) & (RHOSf.^2>=Rfillet.^2) & (pmf*Y >= pmf*(mSecantVec(iFillet)*X + bSecantVec(iFillet))) &...
%             (RHOS>=ID/2. & RHOS <= OD/2. & ((Y <= slopeB*X & Y >= -slopeA*X & X > 0) | (Y >= slopeB*X & Y <= -slopeA*X & X < 0))));
%         maskFillet = double(maskFillet);
            
            
%         if iFillet <= 2
%             maskFillet = ( X > 0 & (RHOSf.^2>=Rfillet.^2) & (Y >= mSecantVec(iFillet)*X + bSecantVec(iFillet)) &...
%                 (RHOS>=ID/2. & RHOS <= OD/2. & ((Y <= slopeB*X & Y >= -slopeA*X & X > 0) | (Y >= slopeB*X & Y <= -slopeA*X & X < 0))));
%             maskFillet = double(maskFillet);
%         else
%             maskFillet = ( X > 0 & (RHOSf.^2>=Rfillet.^2) & (Y <= mSecantVec(iFillet)*X + bSecantVec(iFillet)) &...
...
%         end
        ...
        for iInterior = 1:length(grayInds)
            ...
            subpixel(  (RHOSupf.^2>=Rfillet.^2) & (pmf*Yup >= pmf*(mSecantVec(iFillet)*Xup + bSecantVec(iFillet))) &...
                (RHOSup>=ID/2. & RHOSup <= OD/2. & ((Xup <= slopeB*Yup & Xup >= -slopeA*Yup & Yup > 0) | (Xup >= slopeB*Yup & Xup <= -slopeA*Yup & Yup < 0)))) = 1;
