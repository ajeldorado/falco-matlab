...
% Set default values of input parameters
  ...
  if norm == 1
  	bm.wf = bm.wf .* prop_shift_center(prop_rectangle(bm, sx, sy, 'cx', cx, 'cy', cy, 'rotation', rot, 'norm'), 'inv');
  else
  	bm.wf = bm.wf .* prop_shift_center(prop_rectangle(bm, sx, sy, 'cx', cx, 'cy', cy, 'rotation', rot), 'inv');
  end
