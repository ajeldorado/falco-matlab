...
% Multiplies the wavefront in bm by an elliptical dark obscuration
%
% Outputs:
...
% rx   = X radius of obscuration (meters unless 'norm' is set)
% ry   = Y radius of obscuration (meters unless 'norm' is set)
%
% Optional inputs:
...
% Set default values of input parameters
  ...
  if ( norm == 1 )
	bm.wf = bm.wf .* prop_shift_center(prop_ellipse(bm, rx, ry, 'cx', cx, 'cy', cy, 'dark', 'rotation', rot, 'norm'), 'inv');
  else
	bm.wf = bm.wf .* prop_shift_center(prop_ellipse(bm, rx, ry, 'cx', cx, 'cy', cy, 'dark', 'rotation', rot ), 'inv');
  end
