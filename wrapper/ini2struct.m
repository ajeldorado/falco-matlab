...
while ~feof(f)                          % and read until it ends
    ...
    if ( s(1)=='[' ) && (s(end)==']' )
        ...
        if ~isempty(CurrMainField)
            ...
            MainFields = strsplit(par, '.');
            s_MainFields = size(MainFields, 2);
            if s_MainFields == 1
              Result.(CurrMainField).(genvarname(MainFields{1})) = eval(val);
            elseif s_MainFields == 2
              Result.(CurrMainField).(genvarname(MainFields{1})).(genvarname(MainFields{2})) = eval(val);
            elseif s_MainFields == 3
              Result.(CurrMainField).(genvarname(MainFields{1})).(genvarname(MainFields{2})).(genvarname(MainFields{3})) = eval(val);
            elseif s_MainFields == 4
              Result.(CurrMainField).(genvarname(MainFields{1})).(genvarname(MainFields{2})).(genvarname(MainFields{3})).(genvarname(MainFields{4})) = eval(val);
            end
        else
            ...
            s_MainFields = size(MainFields, 2);
            if s_MainFields == 1
              Result.(genvarname(MainFields{1})) = eval(val);
            elseif s_MainFields == 2
              Result.(genvarname(MainFields{1})).(genvarname(MainFields{2})) = eval(val);
            elseif s_MainFields == 3
              Result.(genvarname(MainFields{1})).(genvarname(MainFields{2})).(genvarname(MainFields{3})) = eval(val);
            elseif s_MainFields == 4
              Result.(genvarname(MainFields{1})).(genvarname(MainFields{2})).(genvarname(MainFields{3})).(genvarname(MainFields{4})) = eval(val);
            end
        end
    end
end
