...
%% Step 5: Initialize the system and get the E-field at the Lyot plane
...
%% Compute the E-fields at the Lyot planes (note that they are computed only to the size of the Lyot stop opening)

ElyotCubeCompact = zeros(mp.P4.compact.Narr,mp.P4.compact.Narr,mp.Nsbp);
ElyotCubeFull = zeros(mp.P4.full.Narr,mp.P4.full.Narr,mp.Nsbp);

%--Get the starlight E-field at the Lyot plane
mp.flagElyot = true;
modvar.whichSource = 'star';
for si=1:mp.Nsbp
    modvar.sbpIndex = si;
    ElyotCubeCompact(:,:,si) = model_compact(mp, modvar);
    
    modvar.wpsbpIndex = 1;
    ElyotCubeFull(:,:,si) = model_full(mp, modvar);
end 

%% Plot the E-fields at the Lyot planes
for si=1:mp.Nsbp
    figure(1); imagesc(abs(ElyotCubeCompact(:,:,si))); axis xy equal tight; colorbar; drawnow;
    figure(2); imagesc(angle(ElyotCubeCompact(:,:,si))); axis xy equal tight; colorbar; colormap hsv; drawnow;

    figure(11); imagesc(abs(ElyotCubeFull(:,:,si))); axis xy equal tight; colorbar; drawnow;
    figure(12); imagesc(angle(ElyotCubeFull(:,:,si))); axis xy equal tight; colorbar; colormap hsv; drawnow;
