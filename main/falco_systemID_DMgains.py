...
class SSM(object):
	...
	def __init__(self, Jacobian1, Jacobian2, Q0, Q1, R0, R1, R2, n_observ=4):
		...
		self.gain1 = tf.Variable(np.ones(Jacobian1.shape[1]), dtype=tf.float64)
		self.G1_real = tf.matmul(Jacobian1.real, tf.diag(self.gain1))
		self.G1_imag = tf.matmul(Jacobian1.imag, tf.diag(self.gain1))
...
#		R = tf.tensordot(tf.expand_dims(self.R0 + self.R1*contrast_p + self.R2*contrast_p**2, axis=-1), tf.ones((1, self.num_pix), dtype=tf.float64), axes=[[-1], [0]]) + 1e-24	
	...
	def get_params(self):
		...
		# return [self.G1_real, self.G1_imag, self.G2_real, self.G2_imag,
		# 		self.q0, self.q1, self.r0, self.r1, self.r2]
		return [self.gain1, self.G2_real, self.G2_imag,
				self.q0, self.q1, self.r0, self.r1, self.r2]
...
def LSEnet(model, Ip, u1p, u2p):
	...
	# field_angle = tf.angle(Enp_pred_new)
	# Enp_pred_new = tf.cast(tf.sqrt(tf.abs(Ip[:, 0, :])), tf.complex128) * (tf.cast(tf.cos(field_angle), tf.complex128) + 1j * tf.cast(tf.sin(field_angle), tf.complex128))
...
class vl_net:
	def __init__(self, Q0=1e-14, Q1=0.05, R0=1e-14, R1=1e-9, R2=0.2,
		...
		# process noises covariance: Q = Q0 + Q1 * mean(|dEc|^2)
		# observation noises covariance: R = R0 + R1 * probe_contrast + R2 * probe_contrast * d_probe_contrast
		...
		sio.savemat(path2data+'jacStructLearned.mat', {'G1': G1,
										'G2': G2,
										'gain1': np.squeeze(np.ones(G1.shape[1], dtype=np.float)),
		...
		# MSE = -elbo
		# MSE = tf.reduce_sum(tf.abs(Enp_pred-Enp_est)**2)
		...
		train_Jacobian = tf.train.AdamOptimizer(learning_rate=learning_rate, 
												beta1=0.99, beta2=0.9999, epsilon=1e-08).minimize(-elbo, var_list=params_list[0:1])
...
def linear_vl(net, lr=1e-7, lr2=1e-2, epoch=10, print_flag=False):
	...
	data_train = sio.loadmat( (path2data+'data_train.mat'))
	u1_train = data_train['data_train']['u1'][0, 0]
	u2_train = np.array(data_train['data_train']['u2'][0, 0], dtype=np.float)
	u1p_train = data_train['data_train']['u1p'][0, 0]
	u2p_train = np.array(data_train['data_train']['u2p'][0, 0], dtype=np.float)
	image_train = data_train['data_train']['I'][0, 0]
	...
	with tf.Session() as sess:
		...
		net.model.gain1.load(np.squeeze(gain1))
		...
		mse = sess.run(net.MSE, feed_dict={net.Enp_old: Enp_est_values[0:n_step-1, :], net.P_old: P_est_values[0:n_step-1, :, :, :],
		...
		for param in [net.model.G1_real, net.model.G1_imag, net.model.G2_real, net.model.G2_imag] + net.params_list[-5::]:
			params_values_list.append(sess.run(param))
		gain1_values = np.squeeze(sess.run(net.model.gain1))
	...
	sio.savemat(path2data+'jacStructLearned.mat', {'G1': params_values_list[0]+1j*params_values_list[1],
										...
										'gain1': gain1_values,
										'noise_coef': np.squeeze(np.exp(np.array(params_values_list[-5::])))})
