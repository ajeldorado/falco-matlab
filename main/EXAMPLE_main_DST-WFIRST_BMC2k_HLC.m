...
mp.t_metal_nm_vec = 0:mp.dt_metal_nm:300;%120; %150; %--nickel thickness range and sampling (nm)
mp.dt_diel_nm = 1;%2/10; %--thickness step size for FPM dielectric layer  (nm)
mp.t_diel_nm_vec = 0:mp.dt_diel_nm:10; %--PMGI thickness range and sampling (nm)

mp.dm8.V0coef = 300;%100;%100/1.42; % Nominal Nickel layer thickness [nm] (the 1.42 disappears when the neighboring actuators are considered)
mp.dm9.V0coef = 0;%390;%240/1.42; % Nominal PMGI layer thickness [nm] (the 1.42 disappears when the neighboring actuators are considered)
